...
public function exportToCSV($filters = []) {
    try {
        ...
        // Headers
        $headers = [
            'ID', 'Nombre Completo', 'RFC', 'CURP', 'Tipo Persona', 
            'Régimen Fiscal', 'Dirección', 'Colonia', 'Delegación/Municipio', 
            'CP', 'Estado', 'País', 'Email', 'Teléfono', 'Teléfono 2', 
            'Extensión', 'Alto Riesgo', 'Fideicomitente', 'Fideicomisario', 
            'Estado', 'Fecha Creación', 'Usuario Creó', 'Fecha Modificación', 
            'Usuario Modificó', 'Comentarios'
        ];
...
private function arrayToCsv($array) {
    ...
    private function validateClienteData($data, $excludeId = null) {
        ...
        if (!empty($data['cp']) && !preg_match('/^[0-9]{5}$/', $data['cp'])) {
            $errors[] = 'Código postal inválido (5 dígitos)';
        }
    ...
    private function compareChanges($oldData, $newData) {
        ...
        $fieldsToCompare = [
            'nombres', 'paterno', 'materno', 'rfc', 'curp',
            'calle', 'nroint', 'nroext', 'cp', 'colonia',
            'delegacion', 'edo', 'emal', 'tel', 'tel2',
            'tipo_persona', 'regimen_fiscal', 'altoriesg',
            'fideicomitente', 'fideicomisario', 'activo'
        ];
