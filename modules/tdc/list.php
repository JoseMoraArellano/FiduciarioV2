...
$canView = $isAdmin 
    || $permissions->hasPermission($userId, 'tdc', 'lire')
    || $session->hasPermission('catalogos', 'lire', 'tdc');

$canCreate = $isAdmin 
    || $permissions->hasPermission($userId, 'tdc', 'creer')
    || $session->hasPermission('catalogos', 'creer', 'tdc');

$canEdit = $isAdmin 
    || $permissions->hasPermission($userId, 'tdc', 'modifier')
    || $session->hasPermission('catalogos', 'modifier', 'tdc');
$canDelete = $isAdmin 
    || $permissions->hasPermission($userId, 'tdc', 'supprimer')
    || $session->hasPermission('catalogos', 'supprimer', 'tdc');
...
<div x-data="tdcController()" x-init="init()" class="p-6 space-y-6">
    ...
    <div class="bg-white rounded-lg shadow p-4">
        <form method="GET" action="" class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
            <input type="hidden" name="mod" value="tdc">
            <div>
            ...
            <div class="flex gap-2">
                ...
                <a href="catalogos.php?mod=tdc" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300">Limpiar</a>
            </div>
        </form>
    </div>
...
//                    tdEstado.innerHTML = r.activo ? '<span class="inline-flex items-center px-2 py-1 rounded text-xs bg-green-100 text-green-800">Activo</span>' : '<span class="inline-flex items-center px-2 py-1 rounded text-xs bg-red-100 text-red-800">Inactivo</span>';
        ...
        openEditModal(id) {
            ...
            fetch(`modules/tdc/actions.php?action=get&id=${id}`)
                .then(r => r.json())
                .then(resp => {
