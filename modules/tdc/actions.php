...
$canView = $permissions->hasPermission($userId,'tdc', 'lire');
$canCreate = $permissions->hasPermission($userId,'tdc', 'creer');
$canEdit = $permissions->hasPermission($userId,'tdc', 'modifier');
$canDelete = $permissions->hasPermission($userId,'tdc', 'supprimer');
...
switch ($action) {
    ...
    try {
                    $stmtCheck = $db->prepare("SELECT COUNT(*) as total FROM t_tdc WHERE DATE(fecha) = DATE(?)");
        ...
        $stmt = $db->prepare("INSERT INTO t_tdc (fecha, tasa, fecha_captura, hora_captura, usuario) 
                      VALUES (?, ?, NOW(), ?, ?)");              
        $stmt->execute([
            $fecha,
            $valor,
    ...
    break;
     ...
     case 'update':
        ...
        if ($existe['total'] > 0) {
            ...
            $stmt = $db->prepare("UPDATE t_tdc SET fecha = ?, tasa = ? WHERE id = ?");
    ...
    case 'delete':
        ...
        try {
            $stmt = $db->prepare("DELETE FROM t_tdc WHERE id = ?");
            $stmt->execute([$id]);
    ...
    case 'get':
        ...
        try {
            $stmt = $db->prepare("SELECT * FROM t_tdc WHERE id = ?");
            $stmt->execute([$id]);
