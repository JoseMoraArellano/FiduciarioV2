...
$canView = $isAdmin 
    || $permissions->hasPermission($userId, 'patrimonios', 'lire')
    || $session->hasPermission('catalogos', 'lire', 'patrimonios');

$canCreate = $isAdmin 
    || $permissions->hasPermission($userId, 'patrimonios', 'creer')
    || $session->hasPermission('catalogos', 'creer', 'patrimonios');

$canEdit = $isAdmin 
    || $permissions->hasPermission($userId, 'patrimonios', 'modifier')
    || $session->hasPermission('catalogos', 'modifier', 'patrimonios');
$canDelete = $isAdmin 
    || $permissions->hasPermission($userId, 'patrimonios', 'supprimer')
    || $session->hasPermission('catalogos', 'supprimer', 'patrimonios');
...
try {
    $query = "SELECT id, nombre, activo FROM t_cat_patrimonios";
...
<div x-data="honoController()" x-init="init()" class="p-6 space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-semibold text-gray-800"><i class="mr-2"></i> Gestión de patrimonios</h1>
        </div>
        <?php if ($canCreate): ?>
        <button @click="openCreateModal()" class="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">
            <i class="fas fa-plus"></i> Nuevo Patrimonio
        </button>
        <?php endif; ?>
    </div>
            ...
            <table class="min-w-full divide-y divide-gray-200" role="table" x-ref="table">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer" @click="sortBy('nombre')">
                            <div class="">Nombre <span x-show="sort.column === 'nombre'"> <template x-if="sort.desc"><i class="fas fa-sort-down"></i></template><template x-if="!sort.desc"><i class="fas fa-sort-up"></i></template></span></div>
                        </th>
                ...
                <tbody class="bg-white divide-y divide-gray-200" x-ref="tbody">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <?php endif; ?>                        
                        <td class="px-4 py-3 text-sm text-gray-700 text-center"><?= htmlspecialchars($row['nombre'] ?? ''); ?></td>                       
                        <?php if ($isAdmin): ?>
        ...
        <form @submit.prevent="submitForm" class="p-4 space-y-4">
            ...
            <div>
                ...
                <input 
                    ...
                    placeholder="Ejemplo: Patrimonio básico">
                <p class="text-xs text-gray-500 mt-1">Descripción del patrimonio</p>
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <p class="text-xs text-gray-500">Marque si el patrimonio está activo</p>
            </div>
    ...
    return {
        ...
        renderRows() {
            ...
            for (const r of this.registros) {
                ...
                tdNombre.className = 'px-4 py-3 text-sm text-gray-700 text-center';
        ...
        openCreateModal() {
            ...
            this.modal.title = 'Nuevo Patrimonio';
            this.modal.actionText = 'Guardar';
            this.form = { id: '', nombre: '', activo: true }; 
        },

        openEditModal(id) {
            fetch(`modules/patrimonios/actions.php?action=get&id=${id}`)
                .then(r => r.json())
                .then(resp => {
                    if (resp.success) {
                        ...
                        this.modal.open = true;
                        this.modal.title = 'Editar Patrimonio';
        ...
        submitForm() {
            ...
            fetch('modules/patrimonios/actions.php', {
                method: 'POST',
