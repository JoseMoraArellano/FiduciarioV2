...
$canView = $permissions->hasPermission($userId,'cpp', 'lire');
$canCreate = $permissions->hasPermission($userId,'cpp', 'creer');
$canEdit = $permissions->hasPermission($userId,'cpp', 'modifier');
$canDelete = $permissions->hasPermission($userId,'cpp', 'supprimer');
...
switch ($action) {
    ...
    try {
                    $stmtCheck = $db->prepare("SELECT COUNT(*) as total FROM t_cpp WHERE DATE(fecha) = DATE(?)");
        ...
        $stmt = $db->prepare("INSERT INTO t_cpp (fecha, tasa, fecha_captura, hora_captura, usuario) 
                      VALUES (?, ?, NOW(), ?, ?)");              
        $stmt->execute([
            $fecha,
            $valor,
    ...
    break;
     ...
     case 'update':
        ...
        if ($existe['total'] > 0) {
            ...
            $stmt = $db->prepare("UPDATE t_cpp SET fecha = ?, tasa = ? WHERE id = ?");
    ...
    case 'delete':
        ...
        try {
            $stmt = $db->prepare("DELETE FROM t_cpp WHERE id = ?");
            $stmt->execute([$id]);
    ...
    case 'get':
        ...
        try {
            $stmt = $db->prepare("SELECT * FROM t_cpp WHERE id = ?");
            $stmt->execute([$id]);
