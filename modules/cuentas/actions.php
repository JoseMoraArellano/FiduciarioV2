...
case 'create':
    ...
    // Campos LA (leer archivos)
    $la_nombre = !empty($_POST['la_nombre']) ? trim($_POST['la_nombre']) : null;
    $la_concepto = !empty($_POST['la_concepto']) ? trim($_POST['la_concepto']) : null;
    $la_titular = !empty($_POST['la_titular']) ? trim($_POST['la_titular']) : null;
    $la_cuenta = !empty($_POST['la_cuenta']) ? trim($_POST['la_cuenta']) : null;
    ...
    $la_banamex = (isset($_POST['la_banamex']) && $_POST['la_banamex'] == 1) ? 't' : 'f';
    $la_bancomer = (isset($_POST['la_bancomer']) && $_POST['la_bancomer'] == 1) ? 't' : 'f';
    $la_scotia = (isset($_POST['la_scotia']) && $_POST['la_scotia'] == 1) ? 't' : 'f';
    $la_hsbc = (isset($_POST['la_hsbc']) && $_POST['la_hsbc'] == 1) ? 't' : 'f';
    $activo = (!isset($_POST['activo']) || $_POST['activo'] == 1) ? 't' : 'f';
    ...
    try {
        $stmt = $db->prepare("INSERT INTO t_cuentas (
            ...
            la_banamex, la_bancomer, la_scotia, la_hsbc, la_nombre, la_concepto, la_titular, la_cuenta
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
case 'update':
    ...
    $la_banamex = (($_POST['la_banamex'] ?? 0) == 1) ? 't' : 'f';
    $la_bancomer = (($_POST['la_bancomer'] ?? 0) == 1) ? 't' : 'f';
    $la_scotia = (($_POST['la_scotia'] ?? 0) == 1) ? 't' : 'f';
    $la_hsbc = (($_POST['la_hsbc'] ?? 0) == 1) ? 't' : 'f';
    ...
    try {
        // UPDATE con todas las columnas LA incluidas
        $stmt = $db->prepare("UPDATE t_cuentas SET 
            ...
            la_banamex = ?, la_bancomer = ?, la_scotia = ?, la_hsbc = ?,
            la_nombre = ?, la_concepto = ?, la_titular = ?, la_cuenta = ?
            WHERE id = ?");
        
        // 36 valores (35 campos + id al final)
        $stmt->execute([
            $fideicomiso, $nombre_de_banco, $cuenta, $clabe, $categoria,
            $saldo_inicial, $saldo_actual, $formato_cheques, $sucursal, $cliente,
